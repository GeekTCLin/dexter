...
async function fetchBoardList(limit: number): Promise<IndustryBoards> {
  const { total, rows } = await fetchClist(BOARDS_FS, BOARD_FIELDS, limit, 'f3');
  return {
    kind: 'boards',
...
export async function getIndustryBoards(limit: number = DEFAULT_BOARD_LIMIT): Promise<Sourced<IndustryBoards>> {
  const pageLimit = clampLimit(limit, DEFAULT_BOARD_LIMIT);
  const cacheParams = { kind: 'boards', limit: pageLimit };
  ...
  if (Date.now() < openUntil) {
    throw new Error('Eastmoney industry data temporarily unavailable (circuit open)');
  }

  try {
    const result = await fetchBoardList(pageLimit);
    failures = 0;
    openUntil = 0;
    const sourceUrl = 'https://quote.eastmoney.com/center/boardlist.html#industry_board';
    writeCache(CACHE_ENDPOINT, cacheParams, { result }, sourceUrl);
    ...
    if (failures >= FAILURE_THRESHOLD) {
      ...
      failures = 0;
      logger.warn(`[domestic-market] industry circuit opened for ${COOLDOWN_MS / 1000}s`, {
...
export async function getIndustryMembers(
  ...
  const pageLimit = clampLimit(limit, DEFAULT_MEMBER_LIMIT);
  const cacheParams = { kind: 'members', board: trimmed, limit: pageLimit };
  ...
  try {
    const resolved = await resolveBoard(trimmed);
    ...
    failures = 0;
    openUntil = 0;
    const sourceUrl = `https://quote.eastmoney.com/bk/90.${resolved.code}.html`;
    writeCache(CACHE_ENDPOINT, cacheParams, { result }, sourceUrl);
    return { value: result, source: 'eastmoney', sourceUrl };
  } catch (error) {
    failures += 1;
